...
int inet_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
    ...
    printk(KERN_INFO "inet_bind: begin\n");
    /* If the socket has its own bind function then use it. (RAW) */
	if (sk->sk_prot->bind) {
		err = sk->sk_prot->bind(sk, uaddr, addr_len);
        printk(KERN_INFO "inet_bind: proto->bind: %d\n", err);
		goto out;
	}
	err = -EINVAL;
    ...
    printk(KERN_INFO "inet_bind: cp 6 snum: %d\n", snum);
...
out_release_sock:
	release_sock(sk);
out:
    printk(KERN_INFO "inet_bind: ret %d\n", err);
	return err;
}
...
const struct proto_ops inet_tstream_ops = {
	.family		   = PF_INET,
	.owner		   = THIS_MODULE,
	.release	   = inet_release,
	.bind		   = inet_bind,
...
EXPORT_SYMBOL(inet_tstream_ops);
...
static int __init inet_init(void)
{
	...
	if (rc)
		goto out_unregister_ttcp_proto;
